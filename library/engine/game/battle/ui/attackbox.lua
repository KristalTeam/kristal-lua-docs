--[[
    Generated from Kristal\src\engine\game\battle\ui\attackbox.lua

    Source: https://github.com/KristalTeam/Kristal/blob/main/src/engine/game/battle/ui/attackbox.lua
]]

---@meta

---@class AttackBox : Object
---@overload fun(...) : AttackBox
AttackBox = {}

function AttackBox:init(battler, offset, x, y) end

function AttackBox:getClose() end

function AttackBox:hit() end

function AttackBox:miss() end

function AttackBox:update() end

function AttackBox:draw() end

